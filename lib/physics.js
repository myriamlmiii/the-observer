...
const CX = W / 2;
const CY = H / 2;
const entropyRng = createRng("physics:entropy");
...
export function physicsStep(bodies, settings) {
  ...
  for (const a of next) {
    ...
    a.vx = (a.vx + ax * timeScale + (entropyRng() - 0.5) * entropy * 0.035) * 0.998;
    a.vy = (a.vy + ay * timeScale + (entropyRng() - 0.5) * entropy * 0.035) * 0.998;
